...
The platform ingests data from IMM machines via MQTT, uses intelligent caching and pub/sub patterns, and serves real-time data to frontend applications through SSE streams instead of expensive polling.
...
export class RealtimeStreamController {
  @Sse('stream')
  stream(@Query('deviceId') deviceId: string) {
    // Stream events filtered by deviceId
...
export class MQTTService {
  ...
  async handleRealtimeMessage(topic: string, message: Buffer) {
    ...
    // Publish to Redis; SSE subscribers receive via pub/sub

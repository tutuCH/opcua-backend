...
## 4. Differential WebSocket Updates

### Purpose
Reduce WebSocket payload size by sending only changed fields instead of full data objects on each update.
...
### Purpose
Further reduce payload size by using binary serialization instead of JSON.
...
## 7. WebSocket Subscription Options

### Purpose
Allow clients to specify which metrics to receive updates for via WebSocket.
...
3. **Differential WebSocket Updates** (Section 4) - Reduce WebSocket payload

### Phase 2: High Impact
4. **Data Aggregation / Down-sampling API** (Section 3) - Reduce historical data size
5. **WebSocket Subscription Options** (Section 7) - Filter metrics at source

### Phase 3: Nice-to-Have
6. **WebSocket Compression** (Section 5) - Additional payload reduction
...
- WebSocket differential updates should be opt-in via client capability negotiation
...
- **WebSocket update latency**: < 100ms from server to client
- **WebSocket payload size**: < 1KB per update (currently ~5-10KB)

...
import { TimestreamWrite, MeasureValueType, TimeUnit } from '@aws-sdk/client-timestream-write';
...
export class MachineTimestreamService {
  ...
  async loadDemoDataToAwsTimestream() {
    try {
      // Read the CSV file
      const demoData = await fs.readFile(path.join(__dirname, 'iot-demo.csv'), 'utf8');
      ...
      for (let i = 0; i < data.length; i += batchSize) {
        ...
        this.logger.log(`Processed batch ${i/batchSize + 1} of ${Math.ceil(data.length/batchSize)}`);
      }
      
      this.logger.log('Successfully loaded all demo data to Timestream');
      return { success: true, recordsLoaded: data.length };
    } catch (error) {
      this.logger.error(`Failed to load demo data: ${error.message}`, error.stack);
      throw new InternalServerErrorException('Failed to load demo data to Timestream');
    }
  }

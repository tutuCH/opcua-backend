...
export class MqttConnectionService {
  ...
  async createConnection(dto: CreateConnectionDto): Promise<{ message: string; clientId: string }> {
    const clientId = uuidv4();
    
    try {
      // AWS IoT Setup
      const { endpointAddress } = await this.iot.describeEndpoint({ endpointType: 'iot:Data-ATS' }).promise();
      ...
      await this.iot.attachThingPrincipal({
        thingName: clientId,
      ...
      throw new HttpException(`AWS IoT setup failed: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  private connectClient(brokerUrl: string, options: mqtt.IClientOptions): Promise<mqtt.MqttClient> {
    return new Promise((resolve, reject) => {
      const client = mqtt.connect(brokerUrl, options);
      const timeout = setTimeout(() => {
        client.end();
        reject(new HttpException('Connection timeout', HttpStatus.GATEWAY_TIMEOUT));
      }, 10000);
  ...
  async removeConnection(clientId: string): Promise<{ message: string }> {
    ...
    return new Promise((resolve, reject) => {
      client.end(false, (err) => {
        if (err) {
          this.logger.error(`Error disconnecting client ${clientId}: ${err.message}`);
          return reject(new HttpException(`Failed to disconnect client: ${err.message}`, HttpStatus.INTERNAL_SERVER_ERROR));
        }
        ...
        this.iot.deleteThing(params, (iotErr, data) => {
          if (iotErr) {
            this.logger.error(`Error deleting AWS IoT thing for client ${clientId}: ${iotErr.message}`);
            return reject(new HttpException(`Failed to delete machine from AWS IoT: ${iotErr.message}`, HttpStatus.INTERNAL_SERVER_ERROR));
          }
          this.logger.log(`AWS IoT thing ${clientId} deleted successfully`);
          resolve({ message: 'Connection removed and machine deleted successfully' });
        });
      });
    });
  }
